...
func (b *WebSocket) SendRequest(op string, args map[string]interface{}, headers map[string]string, reqId ...string) (*WebSocket, error) {
	...
	if err := b.sendAsJson(request); err != nil {
		log.Println("Failed to send websocket trade request:", err)
		return b, err
	}
	log.Println("Successfully sent websocket trade request.")
	return b, nil
}

func (b *WebSocket) SendTradeRequest(tradeTruest map[string]interface{}) (*WebSocket, error) {
	log.Println("trade request headers:", fmt.Sprintf("%v", tradeTruest["header"]))
	log.Println("trade request op channel:", fmt.Sprintf("%v", tradeTruest["op"]))
	log.Println("trade request msg:", fmt.Sprintf("%v", tradeTruest["args"]))
